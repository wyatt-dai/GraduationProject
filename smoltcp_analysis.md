...
wire模块：
主要实现的就是packet结构体，本身基本是个buffer，通过为其实现的结构体方法进行帧的生成

phy模块：
在mod文件中主要定义了device trait，loopback.rs,tuntap_interface.rs等基本就是对这个trait的实现
```
pub trait Device {
...
```
其中RxToken和TxToken两个令牌是发送和接受函数返回的对象，也就是实际的发送需要等到调用了TxToken的consume方法，而读取也是调用consume后复制到缓冲区，最终poll的实现才完成整个发送接受操作。
...
```
poll函数是触发封装和发送的方法，调用了socket_ingress和socket_egress分别处理发送和接收
```
pub fn poll(
        &mut self,
        timestamp: Instant,
...
```
接收函数比较简单，通过设备receive讲数据包从硬件缓冲区获取为（txtoken,rxtoken），然后进行处理
```
fn socket_egress(
        &mut self,
        device: &mut (impl Device + ?Sized),
